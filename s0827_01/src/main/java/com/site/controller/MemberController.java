...
//		Member member = memberService.findLogin(id, pw); 임의로 이름변경
	...
	@GetMapping("/member/insert03") // 회원가입 페이지 열기
	public String insert03() {
		return "member/insert03";
	}
	
	@PostMapping("/member/insert02") //회원가입02 페이지열기
	public String insert02(
			...
			RedirectAttributes redirect,
			Member m) {
		// Member - id, pw, name, gender, hobby (phone은 없음)
		
		String phone = phone1+"-"+phone2+"-"+phone3;
		m.setPhone(phone);
		
		memberService.save(m);
		System.out.println("memberController 저장: ");
	...
	@GetMapping("/member/list") // 회원가입 페이지 열기
	...
	@GetMapping("/member/update") // 수정페이지 열기
